...
export default function collapseWaveElement(waveElement: WaveElement) {
	...
	const probabilities = waveElement.possiblePatterns.reduce(
		(accu: number[], { frequency }) => [...accu, (accu[accu.length - 1] as number) + frequency / totalProbability],
		[0]
	);
	const value = Math.random();
	const collapsedPattern = waveElement.possiblePatterns.find((_, i) => value <= (probabilities[i + 1] as number)) as Pattern;
