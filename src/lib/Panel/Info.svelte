<script lang="ts">
	...
	$: patternGroups = tilesetInfos.patterns
		.filter((pattern) => pattern.transformationIndex == 0)
		.map((pattern) => ({
			name: pattern.name,
	...
	$: compatiblePatternNames = selectedPattern
		? patternGroups
				.filter((patternGroup) =>
					(tilesetInfos.compatibilityMaps as TilesetCompatibilityMaps).some((compatibilityMap) =>
						patternGroup.transformations.some((pattern) => compatibilityMap.get(selectedPattern).get(pattern))
					)
				)
				.map((patternGroup) => patternGroup.name)
		: [];
	$: incompatiblePatternNames = selectedPattern
		? patternGroups
				.filter((patternGroup) => compatiblePatternNames.indexOf(patternGroup.name) == -1)
				.map((patternGroup) => patternGroup.name)
		: [];
</script>
